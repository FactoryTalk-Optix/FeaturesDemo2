...
public class AlarmBannerLogic : BaseNetLogic
{
    ...
    [ExportMethod]
    public void NextAlarm() => alarmBannerSelector?.OnNextAlarmClicked();

    [ExportMethod]
    public void PreviousAlarm() => alarmBannerSelector?.OnPreviousAlarmClicked();
    ...
    {
        public RetainedAlarmsObjectObserver(Action<IContext> action) => registrationCallback = action;
...
public class AlarmBannerSelector : IDisposable, IReferenceObserver
{
    ...
    {
        retaiendAlarms = new List<NodeId>();
        retainedAlarmsLock = new object();
        ...
        rotationTask = new PeriodicTask(() =>
        {
            lock (retainedAlarmsLock)
            {
                DisplayNextAlarm();
            }
        }, rotationTime.Value, logicNode);
    }
    ...
    public void OnReferenceRemoved(IUANode sourceNode, IUANode targetNode, NodeId referenceTypeId, ulong senderId)
    {
        lock (retainedAlarmsLock)
        {
            int alarmIndex = retaiendAlarms.IndexOf(targetNode.NodeId);
            if (alarmIndex == -1)
            {
    ...
    {
        bool wasRunning = RotationRunning;
        StopRotation();
    ...
    public NodeId CurrentDisplayedAlarmNodeId => currentDisplayedAlarm.Value;
    ...
    {
        int previousIndex = currentIndex - 1 < 0 ? retaiendAlarms.Count - 1 : currentIndex - 1;
    ...
    private void DisplayNextAlarm()
    {
        ...
        int nextIndex = currentIndex + 1 >= retaiendAlarms.Count ? 0 : currentIndex + 1;
        SetCurrentIndex(nextIndex);
        UpdateCurrentDisplayedAlarm();
    }

    private void SetCurrentIndex(int index) => currentIndex = index;
    ...
    private void UpdateRetainedAlarmsCount() => retainedAlarmsCount.Value = retaiendAlarms.Count;
    ...
    private readonly IUAVariable currentDisplayedAlarm;
    private readonly IUAVariable currentDisplayedAlarmIndex;
    private readonly IUAVariable rotationTime;
    private readonly IUAVariable retainedAlarmsCount;
    private readonly IUANode logicNode;
    private bool skipFirstCallBack = false;
    private int currentIndex = 0;
    private readonly List<NodeId> retaiendAlarms;
    private readonly object retainedAlarmsLock;
